...
import re
from feature.utils import java_class_name2smali_name, read_txt, retrive_files_set
...
INTENT_TAGS = ["android.intent.action", "com.android.vending", "android.net", "com.android"]
...
Third_part_libraries = ["L" + lib_cnt.split(";")[0].strip('"').lstrip("/") for lib_cnt in read_txt(path_to_lib_type_1, mode="r")]
...
def get_components(app):
    ...
    for activity in activity_elements:
        if activity.hasAttribute("android:name"):
            ...
            component_names.append(activity_name)
    ...
    for service in service_elements:
        if service.hasAttribute("android:name"):
            ...
            component_names.append(svc_name)
    ...
    for receiver in receive_elements:
        if receiver.hasAttribute("android:name"):
            ...
            component_names.append(receiver_name)
...
def get_providers(app):
    ...
    for provider in providers_elements:
        if provider.hasAttribute("android:name"):
            ...
            providers.append(prov_name + TAG_SPLITTER + prov_extra_info)
    return providers
    ...
    for dex in dexes:
        ...
        for smali_cls in smali_classes:
            ...
            for smali_mth in smali_methods:
                ...
                method_header += ".method " + str(smali_mth.access_flags_string) + " " + str(smali_mth.name) + str(smali_mth.proto)
                for instruction in smali_mth.get_instructions():
                    ...
                    # if check_sensitive_api(class_name + "->" + method_name) or check_suspicious_api(class_name + "->" + method_name):
                    # if check_sensitive_api(class_name + "->" + method_name) or check_suspicious_api(class_name + "->" + method_name)  or check(class_name + "->" + method_name):
                    if check_sensitive_api(class_name + "->" + method_name) or check_suspicious_api(class_name + "->" + method_name)  :
                        # api_info = invoke_type + " " + class_name + "->" + method_name + proto + TAG_SPLITTER + SYS_API + TAG_SPLITTER + method_header
...
def get_sensitive_apis():
    ...
    sensitive_apis = set()
    api_file_path = "/home/yuan/workspace/RLDroid/sensitive_apis.txt"
    # api_file_path = "/home/yuan/workspace/RLDroid/api_deduplicated.txt"
...
def get_api_name(api_info):
    if not isinstance(api_info, str):
        raise TypeError("Expect String, but got {}.".format(type(api_info)))

    invoke_match = re.search(
        r"^([ ]*?)(?P<invokeType>invoke\-([^ ]*?)) (?P<invokeObject>L(.*?);|\[L(.*?);)->(?P<invokeMethod>(.*?))\((?P<invokeArgument>(.*?))\)(?P<invokeReturn>(.*?))$", api_info
    )
...
def get_caller_info(node_tag):
    ...
    method_match = re.match(r"^([ ]*?)\.method\s+(?P<methodPre>([^ ].*?))\((?P<methodArg>(.*?))\)(?P<methodRtn>(.*?))$", method_statement)
